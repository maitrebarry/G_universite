...
class Utilisateur extends Model
{
  ...
  public function save_utilisateur() {
    ...
    if ($type_utilisateur == 0  && isset($nom_prenom) && !empty($nom_prenom)) {
        ...
        $existingEnseignant = $this->FetchAllSelectWhere('enseignant_id, enseignant_email', 'enseignants', 'enseignant_email = :email', [':email' => $nom_prenom]);
        var_dump($existingEnseignant);exit;

        if ($existingEnseignant) {
            // Si l'enseignant existe, vérifier que le mot de passe est différent
            $enseignant_id = $existingEnseignant[0]->enseignant_id;
            $existingUtilisateur = $this->FetchAllSelectWhere('mot_passe', 'utilisateur', 'enseignant_id = :enseignant_id', [':enseignant_id' => $enseignant_id]);
            if ($existingUtilisateur && password_verify($mot_passe, $existingUtilisateur[0]->mot_passe)) {
                // Si le mot de passe est identique, erreur
                $this->set_flash("Le mot de passe doit être différent de celui de l'enseignant avec le même email.", 'danger');
                $this->redirect("Utilisateurs/liste_utilisateur");
                return;
            }
        }
    } else {
        // Vérification dans la table utilisateur pour un utilisateur simple
        $existingUtilisateur = $this->FetchAllSelectWhere('mot_passe', 'utilisateur', 'email_utilisateurs = :email', [':email' => $email_utilisateurs]);

        if ($existingUtilisateur && password_verify($mot_passe, $existingUtilisateur[0]->mot_passe)) {
            // Si le mot de passe est identique, erreur
            $this->set_flash("Le mot de passe doit être différent de celui de l'utilisateur avec le même email.", 'danger');
            $this->redirect("Utilisateurs/liste_utilisateur");
            return;
        }
    }
