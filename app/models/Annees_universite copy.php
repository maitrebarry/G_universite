...
class Annees_universite  extends Model
{
        ...
        if (!empty($periode_en_cours)) {
            ...
            if ($date_actuelle >= $date_fin) {
                ...
                $this->insertion_update_simples(
                    'UPDATE periode SET status = :status WHERE id = :id',
                    [':status' => 'achevé', ':id' => $periode->id]
                );
    ...
    //     public function enregistrementAnne(){
    //         $this->e(extract($_POST));
    //         $insertion = $this->insertion_update_simples('INSERT INTO periode(date_debut,date_fin)
    //          VALUES(:date_debut,:date_fin)',[':date_debut'=>$date_debut,':date_fin'=>$date_fin]);
    //    if ($insertion ==true) {
    //     $this->set_flash('periode ajouter avec succes', 'primary');
    //    }else{
    //     $this->set_flash('periode non ajouter');
    //    }
    // }
