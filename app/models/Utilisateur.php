...
class Utilisateur extends Model
{
    ...
    public function save_utilisateur()
    {
        ...
        if ($type_utilisateur == 0  && isset($nom_prenom) && !empty($nom_prenom)) {
            $existingEnseignant = $this->FetchSelectWhere("*", 'utilisateur', 'enseignant_id=? AND role=?', [$nom_prenom, $role]);
            if ($existingEnseignant != null || !empty($existingEnseignant)) {
                $this->set_flash("Cet utlisateur est deja un  " . strtolower($role), 'warning');
                $this->redirect("Utilisateurs/liste_utilisateur");
                return;
            }
            $existingEnseignants = $this->FetchAllSelectWhere("*", 'utilisateur', 'enseignant_id=?', [$nom_prenom]);
            if ($existingEnseignants != null || !empty($existingEnseignants)) {
                foreach ($existingEnseignants as $enseignant) {
                    if (password_verify($mot_passe, $enseignant->mot_passe)) {
                        $this->set_flash("Le mot de passe doit être différent de celui de l'enseignant avec le même email.", 'warning');
                        $this->redirect("Utilisateurs/liste_utilisateur");
                        return;
                    }
                }
            ...
            $existingUtilisateur = $this->FetchSelectWhere('*', 'utilisateur', 'email_utilisateurs = ?', [$email_utilisateurs]);

            if ($existingUtilisateur != null || !empty($existingUtilisateur)) {
                // Si le mot de passe est identique, erreur
                $this->set_flash("Un utilisateur a deja cet email.", 'warning');
                $this->redirect("Utilisateurs/liste_utilisateur");
                return;
            }
        }
